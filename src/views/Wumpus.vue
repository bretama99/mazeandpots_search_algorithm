<template>
  <div class="container">
    ...
    <div class="formalism-buttons">
      ...
      <button @click="setFormalism('asp')" :class="['formalism-button', { active: formalism === 'asp' }]">
        <i class="fas fa-project-diagram"></i> Answer Set Programming
      </button>
    </div>
  ...
  setup() {
    ...
    function logPercepts() {
      ...
      if (percepts.breeze && percepts.stench) {
        perceptsMessage += 'Breeze and Stench';
      } else if (percepts.breeze) {
        perceptsMessage += 'Breeze';
      } else if (percepts.stench) {
        perceptsMessage += 'Stench';
      } else {
    ...
    function checkForHazards() {
      ...
      if (hasGold(x, y)) {
        addLog(`Robot found gold at (${x}, ${y})!`, 'info');
    ...
    function getCellClass(x, y) {
      ...
      if (visited.value.has(posKey)) {
        classes.push('visited');
      }
      
      if (robotPosition.x === x && robotPosition.y === gameY) {
        classes.push('robot-location');
      }
      
      if (isRevealed.value) {
        // If world is revealed, show real state
        if (hasWumpus(x, gameY) || hasPit(x, gameY)) {
          classes.push('unsafe');
        } else {
          classes.push('safe');
        }
      } else {
        // Otherwise show inferred state
        if (isSafeByCertainty(x, gameY)) {
          classes.push('safe');
        } else if (safeProbability(x, gameY) < 0.2) {
          classes.push('unsafe');
        } else if (safeProbability(x, gameY) < 0.7) {
